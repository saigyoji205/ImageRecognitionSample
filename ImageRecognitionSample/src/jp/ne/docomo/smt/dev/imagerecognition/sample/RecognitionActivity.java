...
/**
 * 画像認識要求画面
 * 画像認識要求を実行して結果を取得するサンプルページ
 */
public class RecognitionActivity extends Activity implements OnClickListener  {

	// API キー
	static final String APIKEY = "394c38426e7475444c6e38416a67384b6e377455527248376b646f3367776f4c396174526f647835587637";
	...
	private class RecognitionAsyncTask extends AsyncTask<ImageRecognitionRequestParam, Integer, ImageRecognitionResultData> {
	    ...
	    protected void onPostExecute(ImageRecognitionResultData resultData) {
	    	...
	    	if(resultData == null){
				...
				List<ImageRecognitionCandidateData> candidateList = resultData.getCandidateDataList();
				...
				for (ImageRecognitionCandidateData candidateData : candidateList) {
					...
					ImageRecognitionDetailData detailData = candidateData.getDetailData();
					sb.append(printDetailData(detailData));

					List<ImageRecognitionSiteData> siteList = candidateData.getSiteDataList();
					if (siteList != null) {
						...
						for (ImageRecognitionSiteData siteData : siteList) {
							...
							sb.append("　　ECサイトの画像のURL：" + siteData.getImageUrl() + "\n");
						}
					} else {
					...
					if (relatedList != null) {
						...
						for (ImageRecognitionRelatedContentData relatedData : relatedList) {
							...
							sb.append("　　関連コンテンツのタイトル：" + relatedData.getTitle() + "\n");
							sb.append("　　関連コンテンツの画像のURL：" + relatedData.getImageUrl() + "\n");
							sb.append("　　関連コンテンツの概要：" + relatedData.getAbstract() + "\n");
						}
					} else {
	...
	public void onClick(View v) {
		...
		case R.id.button_feedback:
			...
			intent.putExtra(INTENT_RECOGNITIONID_KEY, this._resultData.getRecognitionId());
		...
		case R.id.button_candidate:
			...
			String itemid = "";
			List<ImageRecognitionCandidateData> candidateList = this._resultData.getCandidateDataList();
			if (candidateList != null && candidateList.size() > 0) {
				ImageRecognitionCandidateData candidateData = (ImageRecognitionCandidateData)candidateList.get(0);
	...
	private void pushExecButton() {
		...
		requestParam.setFilePath(editText.getText().toString());
